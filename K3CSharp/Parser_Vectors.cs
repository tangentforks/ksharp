    ...
    public partial class Parser
    {
        ...
        private ASTNode ParseComplexVectorElement()
        {
            ...
            else
            {
                ...
                if (result == null)
                {
                    throw new Exception("Failed to parse vector element");
                }
                return result;
            }
        }
        ...
        private ASTNode ParseEnhancedParenthesizedElement()
        {
            if (CurrentToken().Type != TokenType.LEFT_PAREN)
            {
                throw new Exception("Expected '(' for enhanced parenthesized element");
            }
            
            // Look ahead to see if this contains semicolons (indicating complex structure)
            int lookaheadPos = current + 1;
            ...
            while (lookaheadPos < tokens.Count && parenDepth > 0)
            {
                ...
                else if (token.Type == TokenType.SEMICOLON && parenDepth == 1) hasSemicolon = true;

...
namespace K3CSharp.Tests
{
    public class KWrapperComparison
    {
                ...
                foreach (var fileName in testFiles)
                {
                    ...
                    switch (result.Status)
                    {
                        ...
                        case ComparisonStatus.Differed:
                            differed++; Console.WriteLine($"❌ FAIL {fileName}"); break;
        ...
        {
            var normalize = (string s) => string.IsNullOrEmpty(s) ? "" : 
                System.Text.RegularExpressions.Regex.Replace(s.Trim(), @"\s+", " ").TrimEnd(';', ' ');
